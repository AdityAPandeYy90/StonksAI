...
def run_three_filters_screener():
    ...
    print(f"\nScreening entire universe of {len(tickers)} NSE stocks (Three Filters Screener)...")
    results = []
    ...
    for ticker in tickers:
        try:
            ...
            # FILTER 2: Proximity to 60-Day High / Pivot (% below 60D High)
            high_60d = float(high.iloc[-60:].max())
            prox_to_high = float((high_60d - curr_price) / high_60d * 100)

            # FILTER 3: Relative Strength vs Benchmark (Outperformance Score)
            ...
            # 3-Filter Screener Criteria
            passes_screener = (move_60d >= 15.0) and (prox_to_high <= 5.0) and (rs_score > 0) and (adr_20 >= 2.0)

            results.append({
                "Symbol": ticker.replace(".NS", ""),
                "Price (Rs)": round(curr_price, 2),
                "Filter 1: 60D Move (%)": round(move_60d, 2),
                "Filter 2: Dist to Pivot (%)": round(prox_to_high, 2),
                "Filter 3: RS vs Nifty (%)": round(rs_score, 2),
    ...
    # Convert to DataFrame & Sort by Status, RS Score, and Proximity
    df_results = pd.DataFrame(results)
    df_results = df_results.sort_values(by=["Status", "Filter 3: RS vs Nifty (%)", "Filter 2: Dist to Pivot (%)"], 
                                       ascending=[False, False, True])

    # Generate timestamped filenames
    now_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    excel_filename = f"Three_Filters_Breakouts_{now_str}.xlsx"
    csv_filename = f"Three_Filters_Breakouts_{now_str}.csv"
    ...
    print("\n--- THREE FILTERS TOP FOCUS CANDIDATES ---")

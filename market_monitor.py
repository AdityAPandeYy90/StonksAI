...
def fetch_nifty500_tickers():
    print("Fetching Nifty 500 ticker list from NSE...")
    try:
        ...
        print(f"Loaded {len(tickers)} Nifty 500 tickers successfully.")
        return tickers
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Market Monitor">
  <Table x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="80">
   ...
   <Column ss:Width="90"/>
   <Row ss:Height="25">
    <Cell ss:MergeAcross="16" ss:StyleID="HeaderTitle"><Data ss:Type="String">Stockbee Market Monitor 2026 (NSE Nifty 500)</Data></Cell>
   </Row>
   <Row ss:Height="25">
...
"""
    ...
    xml_body = ""
    # Write rows in reverse chronological order (newest first)
...
def process_market_breadth(ticker_data_map):
    print("\nProcessing market breadth metrics...")
    ...
    if not baseline_dates:
        print("Error: Could not establish trading days baseline.")
        return
    ...
    # Evaluate for the last 30 days to build a rich historical workbook
    table_days = 30
    ...
    # Print Table
    print("\n" + "="*95)
    print("                      STOCKBEE MARKET MONITOR (INDIAN NSE NIFTY 500)                      ")
    print("="*95)
    ...
    # Overheating Check (Nifty 500 scale: >= 6 stocks)
    overheat_count = latest['50%_plus_1m']
    overheat_alert = "!! DANGER: Overheated !!" if overheat_count >= 6 else "OK (Normal)"
    print(f"  * Overheating Alert (50%+ in 1M)  : {overheat_count} stocks (Threshold: >=6) -> {overheat_alert}")
    if overheat_count >= 6:
        print("    [ALERT] Market is hyper-extended. Do NOT buy breakouts. Tighten stops and raise cash.")
        
    # Capitulation Check (Nifty 500 scale: >= 100 stocks)
    capit_count = latest['4%_minus']
    capit_alert = "!! PANIC DETECTED (Capitulation) !!" if capit_count >= 100 else "OK (Normal)"
    print(f"  * Capitulation Alert (4%- Today) : {capit_count} stocks (Threshold: >=100) -> {capit_alert}")
    if capit_count >= 100:
    ...
    # Save to CSV
    csv_file = "market_monitor_history.csv"
    file_exists = False
    is_duplicate = False
    try:
        ...
        file_exists = True # Default to exists if file lock or read error
    ...
    generate_styled_excel(daily_stats, "market_monitor_sheet.xls")

    # Today's Breakouts printout
    print("\n--- TODAY'S 4% BREAKOUTS (NSE NIFTY 500) ---")
    sorted_breakouts = sorted(today_breakouts, key=lambda x: x[2], reverse=True)
    if sorted_breakouts:
        print(f"{'Ticker':12} | {'Close':8} | {'% Change':8} | {'Volume':10}")
        print("-"*45)
        for ticker, close, pct, vol in sorted_breakouts[:20]:
            print(f"{ticker:12} | {close:8.2f} | {pct:+7.2f}% | {vol:10,d}")
        if len(sorted_breakouts) > 20:
            print(f"...and {len(sorted_breakouts) - 20} more breakouts.")
    else:

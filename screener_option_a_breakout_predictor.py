...
def run_breakout_predictor():
    ...
    print(f"[2/2] Calculating Breakout Tomorrow Readiness Scores (BRS)...", flush=True)
    for t, data in stock_data_map.items():
        ...
        # 2. Prior Momentum Leaderboard Gate (Prior 3M or 1M)
    ...
    predictor_list.sort(key=lambda x: x['brs_score'], reverse=True)

    # Save to CSV
    os.makedirs("d:/stonks/outputs", exist_ok=True)
    csv_path = "d:/stonks/outputs/breakout_tomorrow_predictor.csv"
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Rank", "Ticker", "Close_Rs", "BRS_Score_100", "VDU_Vol_Ratio", "Is_NR7", "Today_Spread_Pct", "5D_Base_Pct", "Prior_3M_Pct", "Set_Alert_Price_Rs", "Pivot_Entry_Rs", "Stop_Loss_Rs"])
        for rank, item in enumerate(predictor_list, 1):
            writer.writerow([rank, item['ticker'], f"{item['close']:.2f}", f"{item['brs_score']:.1f}", f"{item['vdu_ratio']:.2f}", item['is_nr7'], f"{item['today_spread']:.1f}", f"{item['base_range']:.1f}", f"{item['prior_3m']:.1f}", f"{item['alert_price']:.2f}", f"{item['pivot_price']:.2f}", f"{item['stop_loss']:.2f}"])

    print(f"\n==========================================================================================", flush=True)
    print(f"      OPTION A BREAKOUT TOMORROW PREDICTOR (VDU + NR7 + SPREAD COMPRESSION)               ", flush=True)
    print(f"==========================================================================================", flush=True)
    print(f" Analyzed {len(predictor_list)} Option A candidates for Tomorrow's Breakout Probability!")
    print(f" Full Predictor CSV exported to: outputs/breakout_tomorrow_predictor.csv\n")
    print(f"RANK | {'Ticker':<10} | {'Close (Rs)':<10} | {'BRS Score':<10} | {'Vol Ratio (VDU)':<16} | {'Is NR7?':<8} | {'Today Spread':<12} | {'Set Alert At':<16} | {'Pivot Entry':<12}", flush=True)
    print(f"-"*115, flush=True)
    
    for rank, p in enumerate(predictor_list[:15], 1):
        nr7_str = "YES [NR7]" if p['is_nr7'] else "No"
        star = " *** (HIGHEST BREAKOUT PROBABILITY TOMORROW)" if rank <= 3 else ""
        print(f"#{rank:<3} | {p['ticker']:<10} | Rs.{p['close']:<7.2f} | {p['brs_score']:<9.1f}/100 | {p['vdu_ratio']:<15.2f}x | {nr7_str:<8} | {p['today_spread']:<11.1f}% | Rs. {p['alert_price']:<12.2f} | Rs. {p['pivot_price']:<10.2f}{star}", flush=True)

    print(f"\nBreakout Tomorrow Predictor complete.\n", flush=True)

if __name__ == "__main__":
    run_breakout_predictor()

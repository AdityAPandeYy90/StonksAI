...
# TREE A: USER'S HANDWRITTEN TREE (Analogous Leaf Tiers: FULL, HALF2, HALF1, SKIP)
...
# TREE B: PROPOSED TREE WITH IDENTICAL ANALOGOUS LEAF TIERS (FULL, HALF2, HALF1, SKIP)
def evaluate_tree_b(metrics, is_above_10ema, prior_move_pct):
    ...
    tier = "SKIP"
    if dcr10 >= 1.8 and p25 >= 40:
        tier = "FULL"
    elif dcr10 >= 1.4 and p25 >= 30:
        tier = "HALF2"
...
def main():
    print("==========================================================================", flush=True)
    print("     ANALOGOUS DECISION TREES BACKTEST (10 EMA TRAILING STOP EXITS)      ", flush=True)
    print("==========================================================================", flush=True)
    
    all_tickers = fetch_nse_tickers()
    sample_tickers = all_tickers[:500]
    
    print(f"\nDownloading 5 years of daily chart data for {len(sample_tickers)} liquid symbols...", flush=True)
    ticker_data_map = {}
    completed = 0
    ...
    print("Computing 5-Year daily exchange market breadth metrics...", flush=True)
    daily_metrics = {}
    ...
    print("\nRunning Backtest with 10 EMA Trailing Exit for both Analogous Trees...", flush=True)
    
    tier_weight = {"FULL": 1.0, "HALF2": 0.66, "HALF1": 0.33, "SKIP": 0.0}
    ...
    for t, hist in ticker_history.items():
        ...
        # Calculate daily 10 EMA for the stock
        ema10_map = {}
        if closes:
            k = 2 / 11
            ema = closes[0]
        ...
        for idx_pos, idx in enumerate(indices):
            ...
            if chg1d >= 4.0 and date_str in daily_metrics:
                ...
                # -------------------------------------------------------------
                # TRAILING EXIT LOGIC: Hold until first Daily Close < 10 EMA
                # -------------------------------------------------------------
                exit_idx = None
                holding_days = 0
                ...
                if exit_idx is None:
                    # If position never closed below 10 EMA in data window, exit at final bar
                    exit_idx = indices[-1]
                ...
                # Tree B (Analogous Proposed Tree)
    ...
    # Save detailed analogous backtest trades to new separate CSV file
    analogous_csv = os.path.join(OUTPUT_DIR, "analogous_10ema_backtest_trades.csv")
    print(f"\nWriting trade logs for 10 EMA Trailing Exit to separate file: {analogous_csv}...", flush=True)
    with open(analogous_csv, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        ...
        for tr in tree_b_trades:
            writer.writerow([tr['date'], tr['ticker'].replace('.NS',''), "AnalogousProposedTree", tr['tier'], tr['weight'], tr['holding_days'], f"{tr['raw_return']:.2f}", f"{tr['weighted_return']:.2f}", "WIN" if tr['raw_return'] > 0 else "LOSS"])

    print("\n==========================================================================", flush=True)
    print("  ANALOGOUS DECISION TREES BACKTEST RESULTS (10 EMA TRAILING EXIT)       ", flush=True)
    print("==========================================================================", flush=True)

    def print_tree_summary(trades, name):
        if not trades:
    ...
    print_tree_summary(tree_a_trades, "1. USER'S HANDWRITTEN TREE (Analogous Tiers + 10 EMA Exit)")
    print_tree_summary(tree_b_trades, "2. PROPOSED TREE (Analogous Tiers + 10 EMA Exit)")

    # Save summary report to separate markdown file in outputs/
    report_md = os.path.join(OUTPUT_DIR, "analogous_10ema_backtest_report.md")
    print(f"\nWriting summary markdown report to separate file: {report_md}...", flush=True)
    with open(report_md, 'w', encoding='utf-8') as f:
        f.write("# Analogous Decision Trees Backtest Report (10 EMA Trailing Exit)\n\n")
        f.write(f"Generated on: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("- **Exit Rule:** Trailed every trade using 10-Day EMA until the first Daily Close below 10 EMA.\n")
        f.write("- **Leaf Tiers:** FULL (1.0x), HALF 2 (0.66x), HALF 1 (0.33x), SKIP (0.0x).\n\n")
        ...
        for name, t_list in [("User Handwritten Tree", tree_a_trades), ("Proposed Tree (Analogous)", tree_b_trades)]:
            if t_list:
                tot = len(t_list)

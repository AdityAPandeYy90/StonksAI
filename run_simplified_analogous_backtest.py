...
# TREE A: USER'S HANDWRITTEN TREE (Simplified 3-Tier Leaf: FULL=1.0, HALF=0.5, SKIP=0.0)
def evaluate_tree_a(metrics, is_above_10ema):
    ...
    # Apply 10 EMA modifier: if below 10 EMA -> downgrade
...
# TREE B: PROPOSED TREE (Simplified 3-Tier Leaf: FULL=1.0, HALF=0.5, SKIP=0.0)
...
def main():
    print("==========================================================================", flush=True)
    print("    SIMPLIFIED ANALOGOUS TREES BACKTEST (FULL=1.0, HALF=0.5, 10 EMA EXIT) ", flush=True)
    print("==========================================================================", flush=True)
    ...
    print("\nRunning Backtest with 10 EMA Trailing Exit for both Analogous Trees (FULL=1.0, HALF=0.5)...", flush=True)
    ...
    # Save output to separate CSV
    analogous_csv = os.path.join(OUTPUT_DIR, "simplified_analogous_10ema_trades.csv")
    print(f"\nWriting trade logs for simplified backtest to separate file: {analogous_csv}...", flush=True)
    ...
    print("\n==========================================================================", flush=True)
    print("  SIMPLIFIED ANALOGOUS TREES BACKTEST RESULTS (FULL=1.0, HALF=0.5)      ", flush=True)
    print("==========================================================================", flush=True)
    ...
    print_tree_summary(tree_a_trades, "1. USER'S HANDWRITTEN TREE (FULL=1.0, HALF=0.5)")
    print_tree_summary(tree_b_trades, "2. PROPOSED TREE (FULL=1.0, HALF=0.5)")

    report_md = os.path.join(OUTPUT_DIR, "simplified_analogous_10ema_report.md")
    print(f"\nWriting summary markdown report to separate file: {report_md}...", flush=True)
    with open(report_md, 'w', encoding='utf-8') as f:
        f.write("# Simplified Analogous Decision Trees Backtest Report (FULL=1.0, HALF=0.5)\n\n")
